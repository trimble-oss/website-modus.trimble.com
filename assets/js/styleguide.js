...
const codePen = (elem) => {
  ...
  var data = {
    ...
    js_library: "",
    html_classes: "",
    css_external: "https://modus.trimble.com/css/modus.min-0.9.5.css;https://modus.trimble.com/css/modus-layout.min-0.9.5.css;https://modus.trimble.com/assets/0.5.0/fonts/modus-icons.css;https://fonts.googleapis.com/icon?family=Material+Icons",
    js_external: "https://code.jquery.com/jquery-3.5.1.slim.min.js;https://modus.trimble.com/css/modus-layout.min-0.9.5.js;https://stackpath.bootstrapcdn.com/bootstrap/4.5.0/js/bootstrap.bundle.min.js",
    template: true,
  };
...
$(document).ready(function () {
  ...
  $(".guide-code").each(function (i, block) {
    var prev_html = Pretty($(block).siblings(".guide-sample").html());
  ...
  $(".guide-copy-code").on("click", (e) => {
    var str = $(e.currentTarget).parent().siblings(".guide-sample").html();
    copyToClipboard(str);
  ...
  const drawHover = (e, perm) => {
    ...
    let popoverClasses = "." + elem.prop("class").replace(" ", ".").replace("anatomy-display-static", "").replace("anatomy-display-element", "");
    if (popoverClasses === ".") {
    ...
    const popFontColor = showColors
      ? `<p class="small mb-0" id="popover-c"><strong>${perm ? "font-" : ""}color:</strong> ${rgbToHex(elem.css("color"))}<span class="rounded border border-light ml-1 d-inline-block" style="width: 10px; height: 10px; background: ${elem.css("color")}"></span></p>`
      : "";
    const popBorderRadius = elem.css("border-radius") !== "0px" && showDimensions ? `<p class="small mb-0"><strong>border-radius:</strong> ${elem.css("border-radius")}</p>` : "";
  ...
  const buildRightNav = (navItems, navId) => {
    ...
    if (navItems.length > 0) {
      ...
      $('<li class="nav-item"><a class="nav-link font-weight-bold" href="#top">Top <i class="ml-auto material-icons">vertical_align_top</i></a></li>').appendTo($("#" + navId));
      $(navItems).each((i, e) => {
        const elem = $(e);
  ...
  if (href.includes("code-")) {
    // builds right nav for code tabs
    const elem = $(".tab-pane").not(".active");
    const thisNav = $(elem).attr("id") + "-nav";
    ...
    // builds right nav for guide tabs
    ...
    // builds right nav for foundations section
  ...
  } else if (href.includes("Framework-")) {
    // builds right nav for framework section
    let elem = $(".guide-section");
    const thisNav = $(elem).attr("id") + "-nav";
    const navItems = $(elem).children("h2,h3,h4");
    buildRightNav(navItems, thisNav);
    ...
    // builds right nav for other sections not formatted correctly
